...
CREATE TABLE pacijent_u_bolnici (
    ...
    CONSTRAINT FK_pacijent_u_bolnici_pacijent FOREIGN KEY (id_pacijent) REFERENCES pacijent(id),
    CONSTRAINT FK_pacijent_u_bolnici_bolnica FOREIGN KEY (id_bolnica) REFERENCES bolnica(id)
);
...
INSERT INTO bolnica VALUES (1, 'Opća bolnica "Dr. Ivo Pedišić"', 'J.J. Strossmayera 59','Sisak', '044-530-000', 'obs@obs.hr', 530, 40, 220, 110, 140, 100, 190, 150),
						   ...
						   (5, 'Opća bolnica Zadar', 'Bože Peričića 5', 'Zadar', '023-505-505', 'opca.bolnica.zadar@zd.t-com.hr', 4, 330, 220, 110, 52, 168, 255, 216),
                           ...
                           (12, 'Opća bolnica "Hrvatski ponos"', 'Svetoslava Suronje 12', 'Knin', '022-641-111', 'bolnica@obhpknin.hr', 380, 336, 220, 116, 152, 78, 254, 111),
                           (13, 'Opća bolnica Dubrovnik', 'Roka Mišetića 2', 'Dubrovnik', '020-431-777', 'uprava@bolnica-du.hr', 304, 120, 110, 110, 27, 541, 214, 886),
                           (14, 'Opća bolnica Karlovac', 'Andrije Štampara 3', 'Karlovac', '047-608-100', 'ravnateljstvo@bolnica-karlovac.hr', 354, 356, 222, 235, 51, 111, 112, 712),
						   ...
						   (17, 'Županijska bolnica Čakovec', 'I. G. Kovačića 1E', 'Čakovec', '040-375-444', 'bolnica@ck.t-com.hr', 355, 332, 250, 11, 165, 156, 145, 156),
                           ...
                           (20, 'Opća bolnica Pula', 'Aldo Negri 6','Pula', '052-376-500', 'obpula@obpula.hr', 400, 330, 220, 510, 40, 105, 190, 150);
...
-- 1. Ispis imena, prezimena i datum zaposlenja zaposlenika, ime i prezime darivatelja te datum termina na kojem je pojedini zaposlenik 'uzeo' 
--    krv darivatelju'.
SELECT z.ime AS ime_zaposlenika, z.prezime AS prezime_zaposlenika, z.datum_zaposlenja, d.ime AS ime_darivatelja, 
d.prezime AS prezime_darivatelja, t.datum_i_vrijeme 
FROM zaposlenik z
...
SELECT pacijent.ime, pacijent.prezime, pacijent.krvna_grupa, pacijent_u_bolnici.odjel, pacijent_u_bolnici.soba 
FROM pacijent, pacijent_u_bolnici 
WHERE pacijent.id = pacijent_u_bolnici.id_pacijent AND kolicina >= 3;

-- 3. Ispis imena prijevoznika, odredište isporuke (naziv, adresa, grad), krvna_grupa i količina krvi koja će se isporučiti te broj isporuka.
--    Poredati uzlazno po gradu. 
SELECT p.ime_prijevoznika, b.naziv, b.grad, b.adresa, z.krvna_grupa, SUM(z.kolicina) AS prevezena_kolicina, COUNT(p.id) AS broj_prijevoza 
FROM prijevoznik p, bolnica b, zahtjev z
...
SELECT zaposlenik.*, COUNT(zaposlenik.id) AS broj_termina 
FROM zaposlenik
...
-- 5. Kolicina krvi u zalihama nakon svih termina.
SELECT krvna_grupa, (COUNT(id_zalihe) * termin.kolicina + zalihe.kolicina) AS kolicina_u_zalihama 
FROM termin 
...
SELECT SUM(kolicina) AS kolicina_krvi 
FROM darivatelj 
...
SELECT zaposlenik.ime, zaposlenik.prezime, darivatelj.*  
FROM darivatelj 
INNER JOIN termin ON termin.id_darivatelj = darivatelj.id
...
-- 2. Pogled koji će prikazati darivatelje i njihove maksimalne podatke iz kriterija
...
DELIMITER //
CREATE PROCEDURE ciklus_darivanja_krvi (IN id_darivatelj_d INTEGER, OUT datum_i_vrijeme_d DATETIME)
BEGIN
    ...
    IF spol_darivatelja = 'Ž' AND NOW() >= DATE_ADD(datum_i_vrijeme_doniranja, INTERVAL 4 MONTH) THEN
		UPDATE termin SET datum_i_vrijeme = NOW() WHERE id_darivatelj = id_darivatelj_d;
		SET datum_i_vrijeme_d = NOW();
	ELSEIF spol_darivatelja = 'M' AND NOW() >= DATE_ADD(datum_i_vrijeme_doniranja, INTERVAL 3 MONTH) THEN
		UPDATE termin SET datum_i_vrijeme = NOW() WHERE id_darivatelj = id_darivarelj_d;
        SET datum_i_vrijeme_d = NOW();
	ELSE 
...
END //
DELIMITER ;
CALL ciklus_darivanja_krvi (1, @datum_i_vrijeme_d);
SELECT @datum_i_vrijeme_d FROM DUAL;
-- UPDATE termin SET datum_i_vrijeme = STR_TO_DATE('02.01.2021.', '%d.%m.%Y.') WHERE id_darivatelj = 1;
-- SELECT * FROM termin WHERE id_darivatelj = 1;
...
-- 4. Procedura koja omogućuje bolnici automatsko slanje zahtjeva za određenu krvnu grupu te kolicinu iste
 DELIMITER //
CREATE PROCEDURE slanje_zahtjeva (IN id_bolnica_z INTEGER, OUT krvna_grupa_z VARCHAR(4), OUT kolicina INTEGER)
BEGIN   
...
DELIMITER ;
SET @id_bolnica_z = 1;
CALL slanje_zahtjeva(@id_bolnica_z, @krvna_grupa_z, @kolicina);
SELECT @krvna_grupa_z, @kolicina FROM DUAL;
SELECT * FROM zahtjev;
-- 5. Banka krvi zaprima zahtjev koji šalje bolnica.
DELIMITER //
CREATE PROCEDURE zaprimanje_zahtjeva (IN zahtjev_id INTEGER, OUT trazena_krvna_grupa VARCHAR(3), OUT trazena_kolicina DECIMAL(5,2), 
OUT status_zahtjeva VARCHAR(15))
BEGIN 

	SELECT zahtjev.krvna_grupa, zahtjev.kolicina, zahtjev.status_zahtjeva
    INTO trazena_krvna_grupa, trazena_kolicina, status_zahtjeva 
    FROM zahtjev
    INNER JOIN zalihe ON zalihe.id_banka_krvi = zahtjev.id_banka_krvi
    WHERE zahtjev.id = zahtjev_id
    LIMIT 1;
    
    IF status_zahtjeva = 'U tijeku' THEN
		UPDATE zalihe SET kolicina = kolicina - trazena_kolicina WHERE krvna_grupa = trazena_krvna_grupa ;
		UPDATE zahtjev SET status_zahtjeva = 'Zaprimljen' WHERE id = zahtjev_id;
    ELSEIF trazena_kolicina > 100 THEN
		SET status_zahtjeva = 'Odbijen';
    ELSEIF status_zahtjeva = 'Zaprimljen' THEN
		SIGNAL SQLSTATE '50003'
		SET MESSAGE_TEXT = 'Zahtjev ne može biti zaprimljen više puta.';
	END IF;
END //
DELIMITER ;
CALL zaprimanje_zahtjeva(17, @trazena_krvna_grupa, @trazena_kolicina, @status_zahtjeva);
SELECT @trazena_krvna_grupa, @trazena_kolicina, @status_zahtjeva FROM DUAL;
-- SELECT * FROM zahtjev WHERE id = 17;
-- DROP PROCEDURE zaprimanje_zahtjeva;

-- 6. Bolnica zaprima pošiljku (količina određene krvne grupe) što se nadodaje u zalihe bolnice 
DELIMITER //
CREATE PROCEDURE zaprimanje_posiljke (IN bolnica_id INTEGER, INOUT isporucena_krvna_grupa VARCHAR(3), INOUT isporucena_kolicina DECIMAL(5,2))
BEGIN

	SELECT krvna_grupa, kolicina INTO isporucena_krvna_grupa, isporucena_kolicina FROM zahtjev
    WHERE zahtjev.id_bolnica = bolnica_id;
    
    IF isporucena_krvna_grupa = '+A' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_A_poz = zaliha_krvne_grupe_A_poz + isporucena_kolicina;
	ELSEIF isporucena_krvna_grupa = '-A' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_A_neg = zaliha_krvne_grupe_A_neg + isporucena_kolicina;
	ELSEIF isporucena_krvna_grupa = '+B' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_B_poz = zaliha_krvne_grupe_B_poz + isporucena_kolicina;
	ELSEIF isporucena_krvna_grupa = '-B' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_B_neg = zaliha_krvne_grupe_B_neg + isporucena_kolicina;
	ELSEIF isporucena_krvna_grupa = '+AB' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_AB_poz = zaliha_krvne_grupe_AB_poz + isporucena_kolicina;
	ELSEIF isporucena_krvna_grupa = '-AB' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_AB_neg = zaliha_krvne_grupe_AB_neg + isporucena_kolicina;
	ELSEIF isporucena_krvna_grupa = '+0' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_0_poz = zaliha_krvne_grupe_0_poz + isporucena_kolicina;
	ELSEIF isporucena_krvna_grupa = '-0' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_0_neg = zaliha_krvne_grupe_0_neg + isporucena_kolicina;
	ELSE 
		SIGNAL SQLSTATE '50001'
        SET MESSAGE_TEXT = 'Nepostojeća pošiljka';
	END IF;
END //
DELIMITER ;
CALL zaprimanje_posiljke(1, @isporucena_krvna_grupa, @isporucena_kolicina);
SELECT @isporucena_krvna_grupa, @isporucena_kolicina FROM DUAL;
-- DROP PROCEDURE zaprimanje_posiljke;

-- 7. Automatsko dodijeljivanje vrijednosti atributa 'id_kriterij' u relaciju 'darivatelj' po kriterijima
DELIMITER //
CREATE PROCEDURE dodjeljivanje_id_kriterija (INOUT id_darivatelj INTEGER, OUT id_kriterij INTEGER, OUT ime VARCHAR(20), OUT prezime VARCHAR(20), OUT dob INTEGER,
OUT tezina DECIMAL(4,1), OUT temperatura DECIMAL(3,1), OUT sistolicki_tlak INTEGER, OUT dijastolicki_tlak INTEGER,
OUT puls INTEGER, OUT hemoglobin DECIMAL(5,2))
BEGIN

	SELECT id_kriterij, ime, prezime, dob, tjelesna_tezina, tjelesna_temperatura, sistolicki_krvni_tlak, dijastolicki_krvni_tlak, puls, hemoglobin
    INTO id_kriterij, ime, prezime, dob, tezina, temperatura, sistolicki_tlak, dijastolicki_tlak, puls, hemoglobin
    FROM darivatelj
    WHERE id = id_darivatelj;
    
    IF dob >= 18 AND dob <= 24 AND tezina >= 55 AND tezina <= 100 
		AND temperatura >= 36.1 AND temperatura <= 36.5 AND sistolicki_tlak >= 105 AND sistolicki_tlak <= 132
		AND dijastolicki_tlak >= 73 AND dijastolicki_tlak <= 83 AND puls >= 70 AND puls <= 78 AND hemoglobin >= 119 AND hemoglobin <= 175 THEN
			UPDATE darivatelj SET id_kriterij = 1;
	ELSEIF dob >= 25 AND dob <= 35 AND tezina >= 55 AND tezina <= 100 
		AND temperatura >= 36.5 AND temperatura <= 37.0 AND sistolicki_tlak >= 109 AND sistolicki_tlak <= 134
		AND dijastolicki_tlak >= 75 AND dijastolicki_tlak <= 85 AND puls >= 71 AND puls <= 76 AND hemoglobin >= 119 AND hemoglobin <= 175 THEN
			UPDATE darivatelj SET id_kriterij = 2;
	ELSEIF dob >= 36 AND dob <= 45 AND tezina >= 55 AND tezina <= 100 
		AND temperatura >= 36.5 AND temperatura <= 37.0 AND sistolicki_tlak >= 111 AND sistolicki_tlak <= 137
		AND dijastolicki_tlak >= 78 AND dijastolicki_tlak <= 87 AND puls >= 72 AND puls <= 77 AND hemoglobin >= 119 AND hemoglobin <= 175 THEN
			UPDATE darivatelj SET id_kriterij = 3;
	ELSEIF dob >= 46 AND dob <= 54 AND tezina >= 55 AND tezina <= 100 
		AND temperatura >= 36.5 AND temperatura <= 37.0 AND sistolicki_tlak >= 115 AND sistolicki_tlak <= 142
		AND dijastolicki_tlak >= 80 AND dijastolicki_tlak <= 89 AND puls >= 72 AND puls <= 77 AND hemoglobin >= 119 AND hemoglobin <= 175 THEN
		UPDATE darivatelj SET id_kriterij = 4;
	ELSEIF dob >= 55 AND dob <= 65 AND tezina >= 55 AND tezina <= 100 
		AND temperatura >= 36.1 AND temperatura <= 36.5 AND sistolicki_tlak >= 118 AND sistolicki_tlak <= 147
		AND dijastolicki_tlak >= 82 AND dijastolicki_tlak <= 91 AND puls >= 72 AND puls <= 76 AND hemoglobin >= 119 AND hemoglobin <= 175 THEN
		UPDATE darivatelj SET id_kriterij = 5;
	ELSE 
		SIGNAL SQLSTATE '50000'
        SET MESSAGE_TEXT = 'Darivatelj trenutačno ne može donirati krv jer ne ispunjuje kriterije.';
	END IF;
END //
DELIMITER ;
SET @id_darivatelj = 33;
CALL dodjeljivanje_id_kriterija (@id_darivatelj, @kriterij_id, @ime, @prezime, @dob, @tezina, @temperatura, @sistolicki_tlak, @dijastolicki__tlak, @puls, @hemoglobin);


-- 8. Procedura za povećanje zaliha u banci krvi nakon svakog doniranja
DELIMITER //
CREATE PROCEDURE povecanje_zaliha_banke_nakon_termina (IN krvna_grupa_p VARCHAR(3), OUT ukupna_donirana_kolicina DECIMAL(5,2))
BEGIN
	...
	SELECT SUM(termin.kolicina)
	INTO ukupna_donirana_kolicina
    FROM termin
    INNER JOIN zalihe ON termin.id_zalihe = zalihe.id
    INNER JOIN darivatelj ON termin.id_darivatelj = darivatelj.id
    WHERE darivatelj.krvna_grupa = zalihe.krvna_grupa AND zalihe.krvna_grupa = krvna_grupa_p;
    
    START TRANSACTION;
    UPDATE zalihe SET kolicina = kolicina + ukupna_donirana_kolicina WHERE krvna_grupa = krvna_grupa_p;
	COMMIT;
    SET ukupna_donirana_kolicina = ukupna_donirana_kolicina;
END //
DELIMITER ;
CALL povecanje_zaliha_banke_nakon_termina('+A', @ukupna_donirana_kolicina);
SELECT zalihe.*,  @ukupna_donirana_kolicina FROM zalihe WHERE krvna_grupa = '+A';

DROP PROCEDURE povecanje_zaliha_banke_nakon_termina;
-- 9. Procedura koja ažurira zalihe krvi u bolnicama prema količini krvi koja je dana pacijentu
DELIMITER //
CREATE PROCEDURE zalihe_u_bolnicama (IN pacijent_id INTEGER, INOUT id_bolnica_b INTEGER, INOUT naziv_b VARCHAR(50), INOUT zaliha_A_poz  DECIMAL(5,2), INOUT zaliha_A_neg DECIMAL(5,2), INOUT zaliha_B_poz DECIMAL(5,2), 
INOUT zaliha_B_neg  DECIMAL(5,2), INOUT zaliha_AB_poz DECIMAL(5,2), INOUT zaliha_AB_neg DECIMAL(5,2), INOUT zaliha_0_poz DECIMAL(5,2), INOUT zaliha_0_neg DECIMAL(5,2), 
INOUT krvna_grupa_p VARCHAR(3), INOUT kolicina_krvi DECIMAL(3,2))
BEGIN

	SELECT b.id, b.naziv, b.zaliha_krvne_grupe_A_poz, b.zaliha_krvne_grupe_A_neg, b.zaliha_krvne_grupe_B_poz, b.zaliha_krvne_grupe_B_neg, b.zaliha_krvne_grupe_AB_poz, 
    b.zaliha_krvne_grupe_AB_neg, b.zaliha_krvne_grupe_0_poz, b.zaliha_krvne_grupe_0_neg, p.krvna_grupa, pb.kolicina 
    INTO id_bolnica_b, naziv_b, zaliha_A_poz, zaliha_A_neg, zaliha_B_poz, zaliha_B_neg, zaliha_AB_poz, zaliha_AB_neg, zaliha_0_poz, zaliha_0_neg, krvna_grupa_p, kolicina_krvi
    FROM pacijent_u_bolnici pb 
    INNER JOIN bolnica b ON pb.id_bolnica = b.id
    INNER JOIN pacijent p ON pb.id_pacijent = p.id
    WHERE p.id = pacijent_id;   
    
    IF krvna_grupa_p = '+A' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_A_poz = zaliha_krvne_grupe_A_poz - kolicina_krvi;
		SET zaliha_A_poz = zaliha_A_poz - kolicina_krvi;
	ELSEIF krvna_grupa_p = '-A' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_A_neg = zaliha_krvne_grupe_A_neg - kolicina_krvi;
		SET zaliha_A_neg = zaliha_A_neg - kolicina_krvi;
    ELSEIF krvna_grupa_p = '+B' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_B_poz = zaliha_krvne_grupe_B_poz - kolicina_krvi;
		SET zaliha_B_poz = zaliha_B_poz - kolicina_krvi;
    ELSEIF krvna_grupa_p = '-B' THEN 
		UPDATE bolnica SET zaliha_krvne_grupe_B_neg = zaliha_krvne_grupe_B_neg - kolicina_krvi;
		SET zaliha_B_neg = zaliha_B_neg - kolicina_krvi;
    ELSEIF krvna_grupa_p = '+AB' THEN 
		UPDATE bolnica SET zaliha_krvne_grupe_AB_poz = zaliha_krvne_grupe_AB_poz - kolicina_krvi;
		SET zaliha_AB_poz = zaliha_AB_poz - kolicina_krvi;
    ELSEIF krvna_grupa_p = '-AB' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_AB_neg = zaliha_krvne_grupe_AB_neg - kolicina_krvi;
		SET zaliha_AB_neg = zaliha_AB_neg - kolicina_krvi;
    ELSEIF krvna_grupa_p = '+0' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_0_poz = zaliha_krvne_grupe_0_poz - kolicina_krvi;
		SET zaliha_0_poz = zaliha_0_poz - kolicina_krvi;
    ELSEIF krvna_grupa_p = '-0' THEN
		UPDATE bolnica SET zaliha_krvne_grupe_0_neg = zaliha_krvne_grupe_0_neg - kolicina_krvi;
		SET zaliha_0_neg = zaliha_0_neg - kolicina_krvi;
	END IF;
END //
DELIMITER ;
CALL zalihe_u_bolnicama (4, @id_bolnica_b, @naziv_b, @zaliha_A_poz, @zaliha_A_neg, @zaliha_B_poz, @zaliha_B_neg, @zaliha_AB_poz, @zaliha_AB_neg, @zaliha_0_poz, @zaliha_0_neg, 
@krvna_grupa_p, @kolicina_krvi);
SELECT @id_bolnica_b, @naziv_b, @zaliha_A_poz, @zaliha_A_neg, @zaliha_B_poz, @zaliha_B_neg, @zaliha_AB_poz, @zaliha_AB_neg, @zaliha_0_poz, @zaliha_0_neg, 
@krvna_grupa_p, @kolicina_krvi FROM DUAL;

-- 10. Procedura koja postavlja status termina ovisno o tome je li došlo do transakcije između zaliha banke krvi i termina. 
-- Napomena: u obzir uzima ukupno doniranu količinu pojedine grupe, pozivajući proceduru 'povecanje_zaliha_banke_nakon_termina'
DELIMITER //
CREATE PROCEDURE dodjeljivanje_statusa_terminu (IN krvna_grupa_p VARCHAR(3))
BEGIN

    DECLARE kolicina_krvi_zalihe DECIMAL(6,2);
    DECLARE status_t VARCHAR(10);
    DECLARE ukupna_donirana_kolicina DECIMAL(5,2);
    
    CALL povecanje_zaliha_banke_nakon_termina(krvna_grupa_p, ukupna_donirana_kolicina);
    
    SELECT kolicina
    INTO kolicina_krvi_zalihe
	FROM zalihe
	WHERE krvna_grupa = krvna_grupa_p;
    
    CREATE TEMPORARY TABLE IF NOT EXISTS termin_status (
	  id INT AUTO_INCREMENT PRIMARY KEY,
	  krvna_grupa VARCHAR(3) NOT NULL,
	  status_termina VARCHAR(10) NOT NULL,
	  kolicina DECIMAL(5,2) NOT NULL,
      CHECK (status_termina = 'Obavljen' OR status_termina = 'Neobavljen')
	);
   
	IF ukupna_donirana_kolicina <= kolicina_krvi_zalihe THEN
		SET status_t = 'Obavljen';
	ELSE
		SET status_t = 'Neobavljen';
    END IF;
    
	INSERT INTO termin_status (krvna_grupa, status_termina, kolicina) VALUES (krvna_grupa_p, status_t, ukupna_donirana_kolicina);
    
    SELECT * FROM termin_status;
END //
DELIMITER ;
CALL dodjeljivanje_statusa_terminu('-A');
...
# 1. okidač za 2. proceduru - spriječeva da darivatelj ne donira krv pre često

DELIMITER //
CREATE TRIGGER bi_termin
 BEFORE INSERT ON termin
 FOR EACH ROW
BEGIN
 
 CALL ciklus_darivanja_krvi (new.id_darivatelj, new.datum_i_vrijeme);
 
END//
DELIMITER ;

INSERT INTO termin (id_zalihe, id_zaposlenik, id_darivatelj) VALUES (1, 1, 1);
DROP TRIGGER bi_termin;

# 2. okidač za 3. proceduru - nakon unosa termina dodjeljuje nagradu donoru s obzirom na broj njegovih donacija

DELIMITER //
CREATE TRIGGER ai_termin
 AFTER INSERT ON termin
 FOR EACH ROW
BEGIN
 
 CALL nagrada_za_darivatelje (new.id_darivatelj);
 
END//
DELIMITER ;
DROP TRIGGER ai_termin;

# 3. okidač koji ne dozvoljava promjenu krvne grupe darivatelja

DELIMITER //
CREATE TRIGGER bu_darivatelj
 BEFORE UPDATE ON darivatelj
 FOR EACH ROW
BEGIN
 
IF new.krvna_grupa != old.krvna_grupa THEN
	SIGNAL SQLSTATE '40000' SET MESSAGE_TEXT = "Krvna grupa darivatelja se ne može mijenjati.";
END IF;
 
END//
DELIMITER ;

UPDATE darivatelj SET krvna_grupa = 'B+' WHERE id = 1;

# 4. okidač koji ne dozvoljava promjenu krvne grupa pacijenta

DELIMITER //
CREATE TRIGGER bu_pacijent
 BEFORE UPDATE ON pacijent
 FOR EACH ROW
BEGIN
 
IF new.krvna_grupa != old.krvna_grupa THEN
	SIGNAL SQLSTATE '40000' SET MESSAGE_TEXT = "Krvna grupa pacijenta se ne može mijenjati.";
END IF;

END//
DELIMITER ;

UPDATE pacijent SET krvna_grupa = 'A-' where id = 1;
Select * FROM pacijent WHERE id = 1;
DROP TRIGGER bu_pacijent;


# 5. okidač koji ne dozvoljava brisanje termina

DELIMITER //
CREATE TRIGGER bd_termin
BEFORE DELETE ON termin
FOR EACH ROW
BEGIN

	SIGNAL SQLSTATE '40000'
	SET MESSAGE_TEXT = 'Ne možeš obrisati termin.';

END//
DELIMITER ;

DROP TRIGGER bd_termin;
DELETE FROM termin WHERE id = 2;

# 6 okidač koji ne dozvoljava brisanje zahtjeva sa statusom 'U tijeku'

DELIMITER //
CREATE TRIGGER bd_zahtjev
BEFORE DELETE ON zahtjev
FOR EACH ROW
BEGIN
IF old.status_zahtjeva = 'U tijeku' THEN
SIGNAL SQLSTATE '40000'
SET MESSAGE_TEXT = 'Ne možete obrisati zahtjev.';
END IF;
END//
DELIMITER ;

DROP TRIGGER bd_zahtjev;
DELETE FROM zahtjev WHERE id = 1;

select * FROM zahtjev;

# 7 okidač koji osigurava da je datum zaposlenja prilikom unosa novih zaposlenika manji od trenutnog datuma, ako nije postavlja ga na trenutni datum
...
FOR EACH ROW
BEGIN
IF new.datum_zaposlenja > NOW() AND new.datum_zaposlenja = NULL THEN
	SET new.datum_zaposlenja = NOW();
END IF;
END//
DELIMITER ;

DROP TRIGGER bi_zaposlenik;
INSERT INTO zaposlenik VALUES (36, 'Mislav', 'Krivić', STR_TO_DATE('01.11.1999.', '%d.%m.%Y.'), 'Ul. Kaktusa 15', 'Zagreb', '075-589-7895', 'mkrivic@ob-pula', STR_TO_DATE('01.11.2023.', '%d.%m.%Y.'));
-- DELETE FROM zaposlenik WHERE id = 36;
SELECT * FROM zaposlenik;

# 8. okidač koji ne dozvoljava da zalihe krvi ne padnu ispod 100.0

DELIMITER //
CREATE TRIGGER bu_zalihe
BEFORE UPDATE ON zalihe
FOR EACH ROW
BEGIN

IF new.kolicina < 100.0 THEN
SIGNAL SQLSTATE '40000'
 SET MESSAGE_TEXT = 'Količina na zalihama ne može biti manja od 100.0!';

END IF;
END//
DELIMITER ;

DROP TRIGGER bu_zalihe;
-- UPDATE zalihe SET kolicina = -950.0 WHERE krvna_grupa = '-B';
-- provjeriti jel radi mni safe mode ne dozvoljava

# 9. okidač koji ne dozvoljava mijenjanje prijevoznika u zahtjevu

DELIMITER //
CREATE TRIGGER bupd_zahtjev
BEFORE UPDATE ON zahtjev
FOR EACH ROW
BEGIN
IF  new.id_prijevoznik != old.id_prijevoznik THEN
	SIGNAL SQLSTATE '40000' SET MESSAGE_TEXT = "Ne možete mijenjati prijevoznika zahtjeva";
END IF;
END//
DELIMITER ;

DROP TRIGGER bupd_zahtjev;
UPDATE zahtjev SET id_prijevoznik = 5 WHERE id = 6;
select * from zahtjev;


# 10. okidač za 7. proceduru - ne dozvoljava unos novog darivatelja ako ne ispunjava sve kriterije

DELIMITER //
CREATE TRIGGER bi_darivatelj
BEFORE INSERT ON darivatelj
FOR EACH ROW
BEGIN
CALL dodjeljivanje_id_kriterija(new.id, new.id_kriterij, new.ime, new.prezime, new.dob, new.tjelesna_tezina, new.tjelesna_temperatura, 
								new.sistolicki_krvni_tlak, new.dijastolicki_krvni_tlak, new.puls, new.hemoglobin);
END//
DELIMITER ;
DROP TRIGGER bi_darivatelj;

INSERT INTO darivatelj VALUES (38, NULL, 'Ana', 'Matošević', '12344482342', 'Ž', STR_TO_DATE('06.12.1995', '%d.%m.%Y.'), 'Aničićev prilaz 5', 'Pula', '087-246-7890', '+A', 27, 59.0, 37.2, 120, 85, 73, 120);
INSERT INTO darivatelj VALUES (39, NULL, 'Ana', 'Matošević', '12344482342', 'Ž', STR_TO_DATE('06.12.1995', '%d.%m.%Y.'), 'Aničićev prilaz 5', 'Pula', '087-246-7890', '+A', 16, 59.0, 36.5, 120, 85, 73, 120);


/*____________________________________________________Autorizacija i autentifikacija _________________________________________________________*/

DROP USER IF EXISTS administrator;
CREATE USER administrator IDENTIFIED BY 'id';
GRANT ALL PRIVILEGES ON *.* to administrator;


DROP USER IF EXISTS zaposlenik;
CREATE USER zaposlenik IDENTIFIED BY 'id';
GRANT SELECT, UPDATE ON baza_banke_krvi.darivatelj TO zaposlenik;
GRANT INSERT ON baza_banke_krvi.darivatelj TO zaposlenik;
GRANT SELECT ON baza_banke_krvi.zalihe TO zaposlenik;


DROP USER IF EXISTS zaposlenik_s_vise_ovlasti;
CREATE USER zaposlenik_s_vise_ovlasti identified BY 'id';
GRANT CREATE ON baza_banke_krvi.darivatelj TO zaposlenik_s_vise_ovlasti;
GRANT SELECT ON baza_banke_krvi.darivatelj TO zaposlenik_s_vise_ovlasti;
GRANT UPDATE ON baza_banke_krvi.darivatelj TO zaposlenik_s_vise_ovlasti;
GRANT DELETE ON baza_banke_krvi.darivatelj TO zaposlenik_s_vise_ovlasti;

GRANT SELECT ON baza_banke_krvi.prijevoznik TO zaposlenik_s_vise_ovlasti;
GRANT SELECT ON baza_banke_krvi.zalihe to zaposlenik_s_vise_ovlasti;
